...
local Window = Rayfield:CreateWindow({
    Name = "Word Hunt 5x5",
    LoadingTitle = "Word Hunt Tool",
    LoadingSubtitle = "by 09092015hoang-afk",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "WordHunt",
        FileName = "WordHuntSettings"
    }
})

local MainTab = Window:CreateTab("Nhập lưới", nil)
local ResultTab = Window:CreateTab("Kết quả", nil)
...
local function FindWordsFromCell(startX, startY)
    ...
    local function dfs(x, y, currentWord, path)
        ...
        if WordList[currentWord] and #currentWord >= 3 then
            table.insert(found, {word = currentWord, start = {startX, startY}})
        end
...
local InputSection = MainTab:CreateSection("Nhập chữ cái")

for i = 1, 5 do
    local rowFrame = Instance.new("Frame")
    rowFrame.Size = UDim2.new(1, 0, 0, 45)
    rowFrame.BackgroundTransparency = 1
    rowFrame.Parent = MainTab
    ...
    for j = 1, 5 do
        local Input = MainTab:CreateInput({
            ...
            Callback = function(Text)
                local val = string.sub(Text, 1, 1):upper()
                if val:match("%a") then
                    Grid[i][j] = val:lower()
                else
                    Grid[i][j] = ""
...
local CalcBtn = MainTab:CreateButton({
    Name = "▶ TÍNH TOÁN",
    Callback = function()
        local allWords = {}
        for i = 1, 5 do
        ...
        for _, item in ipairs(allWords) do
            if not seen[item.word] then
                seen[item.word] = true
                count = count + 1
                resultText = resultText .. string.format("%s (ô %d,%d)\n", item.word, item.start[1], item.start[2])
            end
        end
        
        if count == 0 then
            resultText = "Không tìm thấy từ nào."
        else
            resultText = "Tìm thấy " .. count .. " từ:\n\n" .. resultText
        end
        
        local ResultSection = ResultTab:CreateSection("Danh sách từ")
        local ResultLabel = ResultTab:CreateLabel(resultText)
        
        Rayfield:Notify({
            Title = "Hoàn thành",
            Content = "Đã tìm thấy " .. count .. " từ",
            Duration = 3
        })
    end
})
...
local ClearBtn = MainTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            Title = "Đã xóa",
            Content = "Đã xóa toàn bộ lưới",
            Duration = 2
        })
    end
})

local ReloadBtn = MainTab:CreateButton({
    Name = "TẢI LẠI TỪ ĐIỂN",
    Callback = function()
        WordList = {}
        for _, w in ipairs(fallback) do
            WordList[w] = true
        ...
        Rayfield:Notify({
            Title = "Đã tải lại",
            Content = "Từ điển đã được cập nhật",
            Duration = 2
        })
    end
})

print("Word Hunt 5x5 đã chạy với Rayfield UI") = player
end
